...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // themes that zero `[class] { border-radius }` globally). Generic only:
    // default is SQUARE (the square-corner look is the theme's normal state);
    // we switch to round on evidence — a majority of non-zero radii on
    // shipped card/panel elements (base themes keep them rounded).
